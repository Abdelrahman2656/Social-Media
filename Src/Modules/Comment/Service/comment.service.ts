...
//---------------------------------------------------Create Comment--------------------------------------------------------------
export const createComment = async(req:AppRequest,res:AppResponse,next:AppNext)=>{
//get data from req 
...
let{secure_url,public_id,resource_type}= await cloudinary.uploader.upload(files.attachment[0].path,{
...
if(files?.voice&& files.voice.length > 0){
  ...
  if (!allowedAudioTypes.includes(voiceFile?.mimetype)) {
    return next(new AppError("Only audio files are allowed in 'voice' field", 400));
  }
let{secure_url,public_id,resource_type ,duration}= await cloudinary.uploader.upload(voiceFile.path,{
    folder:`Social-Media/Users/${postExistence.publisher}/Posts/Comment`,
    resource_type:"auto"
})
...
//save response
return res.status(200).json({message:messages.comment.createdSuccessfully,success:true , CommentData:commentCreated})
}
//---------------------------------------------------Get Comment--------------------------------------------------------------
export const  getComment = async(req:AppRequest,res:AppResponse,next:AppNext)=>{
  //get data from req 
  const {postId}=req.params
  //get comment
